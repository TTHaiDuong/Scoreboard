import { timerDb, MatchTimer } from "../components/timer.js"
...
export default function initScoreChannel(io: any, socket: any) {
    ...
    // Operator ngắt kết nối một máy giám định cụ thể
    // Phải có vai trò là "control"
    socket.on("judge:kick", (data: { targetSocketId: string }) => {
        const user = socket.user
        if (!user || user.role !== "control") return

        const targetSocket = io.sockets.sockets.get(data.targetSocketId)
        if (!targetSocket) return

        // Chỉ được kick socket có role là "judge"
        if ((targetSocket as any).user?.role !== "judge") return

        targetSocket.emit("judge:kicked", { reason: "Bị ngắt kết nối bởi operator" })
        targetSocket.disconnect(true)

        // Thông báo cho tất cả trong court
        if (user.courtId) {
            io.to(`court-${user.courtId}`).emit("judge:disconnected", {
