...
import { Score } from "@/scripts/types"
...
export function emptyBreakdown(): ScoreBreakdown {
    return { punch: 0, trunkKick: 0, headKick: 0, spinTrunk: 0, spinHead: 0, gamjeom: 0 }
}

export function calcTotalFromBreakdown(b: ScoreBreakdown, rivalGamjeom: number): number {
    return b.punch * 1 + b.trunkKick * 2 + b.headKick * 3 +
        b.spinTrunk * 4 + b.spinHead * 6 + rivalGamjeom
}
...
export function createEmptyRound(roundNo: RoundResult["roundNo"], durationMs = 120_000): RoundResult {
    return {
        roundNo,
        blueScore: 0,
        redScore: 0,
...
export type MatchConfig = {
    ...
    kyeshiMs: number   // 60_000
    maxGamjeom: number   // 10
    pointGap: number   // 12
...
export type MatchInfo = {
    ...
    rounds: Partial<Record<1 | 2 | 3 | "golden", RoundResult>>
    matchWinner?: Side
