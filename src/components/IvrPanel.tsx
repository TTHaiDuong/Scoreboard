...
function HistoryRow(props: { record: IvrRecord; index: number }) {
    ...
    return (
        <div className="flex items-center gap-[8px] py-[7px]
            ...
            <span className="text-[11px] text-white/25 w-[16px] text-center font-score">
                {props.index + 1}
            </span>
            <span className={`text-[12px] font-medium ${sideColor} w-[32px]`}>{sideLabel}</span>
            <span className="text-[11px] text-white/35">Hiệp {r.roundNo}</span>
            <span className="font-score text-[11px] text-white/30">{r.timeLabel}</span>
