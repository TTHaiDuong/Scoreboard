    ...
    return (
        <div className="flex items-center w-full min-h-[52px] px-[4px] gap-[10px]">
            <button
                ...
                disabled={isLinked}
            >
                <span className="flex-1 text-[15px] font-medium text-[rgb(var(--color-text-primary))]">
                    {props.label}
                </span>
...
export default function MobileSetting(props: {
    ...
    function getPickerInitMs() {
        ...
        return effectiveValue({ roundMs, breakMs, kyeshiMs }[timePickerTarget])
    }
                ...
                <div className="flex-1 overflow-y-auto px-[16px] py-[20px] flex flex-col gap-[24px] bg-[#111]">
                    ...
                    <SettingGroup title="Kết nối & Thiết bị">
                        <NavRow
                            label="Mã bảng điểm"
                            value={
                                <span className="font-score font-bold text-blue-600 text-[16px]">
                                    {props.courtId ?? "–"}
                                </span>
                            }
                    ...
                    <SettingGroup title="Luật thi đấu">
                        ...
                        */}
                        <div className="flex items-center w-full min-h-[58px] px-[4px] gap-[8px]">
                            ...
                            {/* Binding cho trạng thái bật/tắt */}
                            <ChainButton
                                isLinked={pointGapEnabled.isLinked}
                                sheetAvailable={pointGapEnabled.sheetValue !== null}
                                onToggle={() => setPointGapEnabled(toggleLink(pointGapEnabled))}
                            />
